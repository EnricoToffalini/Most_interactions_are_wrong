...
: Operational rules the coders applied for the decisions that most affect the reported counts, condensed from the coding sheet. The preregistered variable definitions are in @tbl-variables. {#tbl-coding-rules}
...
```{r}
#| label: tbl-fp-forced-choice
#| results: asis

print_table_if_exists(
  caption = "Simulation 1 (forced-choice accuracy): false-positive interaction rates by performance scenario and fitted model, with the median model-implied change in the group gap (correct-response units). Fits = converged replications; Sig = replications returning a significant interaction; CI = Wilson 95%.",
  ...
  transform = function(d) data.frame(
    ...
    Sig = d$n_significant,
    `False-positive rate [95% CI]` = fmt_rate_ci(d$false_positive_rate, d$ci_low, d$ci_high),
    `Gap change` = fmt_num(d$median_change_in_group_difference_outcome_units, 2),
    check.names = FALSE
  )
)
```
...
```{r}
#| label: tbl-fp-within-family
#| results: asis

print_table_if_exists(
  caption = "Simulation 2 (within-family link choice): false-positive interaction rates for the condition-by-group term in random-intercept GLMMs, crossing the generating and fitted links. The median coefficient and SE are on the fitted link scale. CI = Wilson 95%.",
...
```{r}
#| label: tbl-fp-sum-score
#| results: asis

print_table_if_exists(
  caption = "Simulation 3 (sum scores): false-positive interaction rates by scenario and analysis scale, with the median model-implied change in the group gap. The latent generating scale is an idealized benchmark available only because theta is known. CI = Wilson 95%.",
  ...
  transform = function(d) data.frame(
    ...
    `False-positive rate [95% CI]` = fmt_rate_ci(d$false_positive_rate, d$ci_low, d$ci_high),
    `Gap change` = fmt_num(d$median_change_in_group_difference_outcome_units, 2),
    Units = d$change_in_group_difference_units,
    check.names = FALSE
  )
)
```
...
print_table_if_exists(
  caption = "Deterministic link-scale decomposition for Simulation 2. The induced product term is evaluated on the fitted link scale; the response-scale difference-in-differences is on the probability scale.",
