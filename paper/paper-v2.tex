...
(linear model) decline at different rates, and the gap between the
groups is much wider at age six than at age ten. The natural reading is
that the group difference shrinks with age, that is, that age and group
interact.
...
the log scale its fitted lines are parallel. Mapped back onto the count
scale, the same model produces two curves that converge with age,
...
narrows as errors decline. The linear model registers this narrowing and
reads it as an interaction. Whether an interaction is present thus
depends on the scale on which effects are assumed to add. Additivity on
the log scale implies non-parallel curves on the count scale, and
additivity on the count scale would imply non-parallel lines on the log
scale.
...
\begin{tcolorbox}[enhanced jigsaw, title={Box 1: Generalized linear models: outcome family and link function}, colback=white, colframe=quarto-callout-note-color-frame, coltitle=black, colbacktitle=quarto-callout-note-color!10!white, opacitybacktitle=0.6, left=2mm, rightrule=.15mm, bottomrule=.15mm, toprule=.15mm, opacityback=0, toptitle=1mm, bottomtitle=1mm, titlerule=0mm, arc=.35mm, breakable, leftrule=.75mm]
