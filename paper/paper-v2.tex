...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, bottomtitle=1mm, rightrule=.15mm, leftrule=.75mm, arc=.35mm, colbacktitle=quarto-callout-note-color!10!white, titlerule=0mm, opacitybacktitle=0.6, breakable, colframe=quarto-callout-note-color-frame, colback=white, title={Box 1: Generalized linear models: outcome family and link function}, toptitle=1mm, toprule=.15mm, left=2mm, opacityback=0, coltitle=black]
...
probit, using simulations to quantify how often an inadequate link turns
a purely additive structure into a statistically significant interaction
and whether standard diagnostics detect the misspecification.
...
encode different additivity baselines (this is narrower and more focused
than removability, as we consider only a specified set of alternative
link functions). In empirical data, where the generating scale is
unknown, we describe interaction conclusions as link-sensitive,
conditional on the fitted scale, or unstable across plausible links.
...
replication, including the lower-performance scenario. Complete fit
success does not imply equal precision: under the matched
chance-corrected model, the Monte Carlo standard deviation of the
estimated age-by-group coefficient was 0.180 in the lower-performance
scenario, compared with 0.115 in the middle-performance scenario and
0.099 in the higher-performance scenario. This greater sampling
variability is consistent with less information about link-scale changes
when expected accuracy lies closer to the chance floor.
