...
\begin{tcolorbox}[enhanced jigsaw, coltitle=black, title={Box 1: Generalized linear models: outcome family and link function}, colback=white, breakable, colbacktitle=quarto-callout-note-color!10!white, opacitybacktitle=0.6, toptitle=1mm, bottomtitle=1mm, titlerule=0mm, opacityback=0, arc=.35mm, rightrule=.15mm, leftrule=.75mm, bottomrule=.15mm, toprule=.15mm, left=2mm, colframe=quarto-callout-note-color-frame]
...
psychological interaction tests. The coding is deliberately descriptive,
based only on what a published report makes observable, so it documents
reporting practice rather than the correctness of any substantive
conclusion.
...
and the worked examples therefore focus on binomial and aggregated
...
2000}; \citeproc{ref-knoblauch2012}{Knoblauch \& Maloney, 2012};
\citeproc{ref-wichmann2001}{Wichmann \& Hill, 2001}). For example, the
3-parameter logistic Item Response Theory model includes an
...
across the three scenarios, close to the nominal 5\%, and is lowest near
the floor where the data carry least information about above-chance
differences. Pseudo-interaction detection is highest where performance
lies near the floor: in the lower-performance scenario the standard
logit rejects 64\% of the time, the standard probit 61\%, and the
Gaussian identity 46\%. Detection falls as performance moves away from
the floor, reaching 34\% for the logit, 20\% for the probit, and 12\%
for the Gaussian identity in the higher-performance scenario. The same
qualitative sensitivity to the chance floor also appears at chance
levels 0.25 and 0.33 (Supplement B).

Near the floor, however, the product term is weakly identified in the
chance-corrected model. As expected accuracy approaches chance, the
curve that maps the above-chance scale onto observed accuracy flattens,
so widely different values on that scale imply almost the same expected
accuracies, and the data cannot distinguish them. In the
lower-performance scenario only 164 of the 300 replications produced a
usable test of the product term, since in the rest the data constrained
the product term too weakly to yield a trustworthy standard error. The
rejection rate is therefore computed on those replications, and since
failures concentrate where the data are least informative about
above-chance differences, they are a selected subset. Restricting all
four models to those 164 replications puts them on identical datasets
and leaves the pattern unchanged. The chance-corrected model rejects in
2\% of them, against 45\% for the Gaussian identity, 63\% for the logit,
...
range. Across the broader scenarios in Supplement B, scale location
affected identity-model rejection much more than sample size, item
count, or latent dispersion. Raw-score additivity can approximate
latent-scale additivity when scores remain away from the bounds.
Compression near a bound can instead create an apparent interaction from
a constant latent effect. The bounded-score probit model can therefore
serve as a useful sensitivity analysis. For claims about interactions
between latent constructs, an explicit measurement model provides a more
direct approach, for example SEM, item-level ordinal models, or
IRT-inspired models. These approaches still depend on assumptions about
thresholds, links, loadings, and latent-response scales, but they make
those assumptions part of the interaction model.
...
Information criteria such as AIC, BIC, WAIC, and LOO compare a supplied
set of models. For link choice, the relevant comparison keeps the
response, family, and structural formula fixed and varies only the link.
This is best viewed as a targeted sensitivity analysis. It has two
important limits. First, the result is restricted to the candidate links
that were fitted. Second, the interaction term is already present in
each model and can absorb part of the curvature produced by the wrong
link. A misspecified model may therefore fit almost as well as the
target model while still placing additivity on the wrong scale. In our
comparisons, the candidate models have the same number of parameters, so
their AIC ranking depends entirely on the log-likelihood.
...
the Poisson family and interaction formula but uses an identity link,
making this a same-family comparison in which only the link changes. In
real research scenarios, however, family and link often covary.

The second used the lower-performance forced-choice scenario from
Simulation 1. Data were generated with a 0.50 chance floor, 20 trials
per participant, intercept -0.80, age effect 0.60, group effect -0.90,
and no age-by-group interaction on the chance-corrected logit scale. The
fitted model was a standard binomial-logit interaction model. It
retained the binomial trial structure but placed the lower asymptote at
0. The chance-corrected model was initialized from the standard-logit
fit and optimized with BFGS, with Nelder--Mead as fallback when needed.

The third scenario reproduced the probit-generated, logit-fitted case
from Simulation 2. Data consisted of repeated binary trials with a
subject random intercept, 15 trials per condition cell, and latent ICC
.30. Fixed effects were intercept 1.50, group effect -1.00, condition
effect -1.00, and no group-by-condition interaction. The fitted model
was the corresponding random-intercept logit GLMM with the same formula.

The fourth scenario was a continuous-predictor version of the same
binary case. It kept the probit generating link, logit fitted link,
group effect, focal-predictor effect, random intercept, and latent ICC,
while replacing the two-level condition with a continuous predictor
sampled from 0 to 1. This allows residual checks that rely on variation
along a continuous predictor.
...
inspect after obtaining a significant interaction. The question is
therefore whether the same model that produces the pseudo-interaction
also shows enough evidence of misspecification to warn the analyst. This
is a demanding test, since the interaction term can already absorb part
of that curvature.

The DHARMa workflow included four checks: residual uniformity,
dispersion, residual quantile patterns over fitted values, and residual
structure over the focal predictor (\citeproc{ref-dunnsmyth1996}{Dunn \&
Smyth, 1996}; \citeproc{ref-hartig2024}{Hartig, 2024}). For continuous
predictors we used a quantile check; for the categorical \(2 \times 2\)
design we used a design-cell comparison. We also applied the
Pregibon-style added-term link check
(\citeproc{ref-pregibon1980}{Pregibon, 1980}). As a calibration check,
the same DHARMa and Pregibon-style procedures were applied to the
corresponding correctly specified interaction model in each replication.

AIC was used only for relative comparison among prespecified candidate
mean models fitted to the same data with the same interaction formula,
keeping family or sampling model fixed as described above and varying
only the link. The chance-floor scenario compared a standard logit model
with a chance-corrected logit model with a design-implied lower
asymptote. We report both the proportion of replications in which AIC
favored the target model and the distribution of \(\Delta\)AIC, defined
as AIC of the misspecified model minus AIC of the target model. Positive
values therefore favor the target link. Because no inconclusive category
was used, the complementary proportion favors the misspecified model
...
confidence interval. `N.A.' indicates that the Pregibon-style added term
is not identifiable in the saturated \(2   imes 2\) interaction model:
the squared population-level linear predictor is exactly collinear with
the existing fixed effects.}{\label{tbl-diagnostic-scenarios}}}
\vspace{-20pt}}
...
Table~\ref{tbl-diagnostic-scenarios} shows that diagnostic sensitivity
varied widely across scenarios. In the count case, the wrong link
produced pseudo-interactions in 96\% of replications. Pregibon detected
the problem in 99\%, while the most sensitive DHARMa check reached 88\%.
In the chance-floor case, the corresponding rates were 59\% for the
pseudo-interaction, 32\% for Pregibon, and at most 6\% for DHARMa. In
the Gamma case, they were 30\%, 34\%, and at most 6\%, respectively.

The logit-probit cases were harder to diagnose. In the saturated
\(2 \times 2\) design, pseudo-interactions occurred in 13\% of
replications and DHARMa detected at most 1\%; Pregibon was not
identifiable. With a continuous predictor, the corresponding rates were
7\% and 0\%, while Pregibon detected 4\%. AIC was more sensitive in both
cases, favoring the target probit model in 95\% and 94\% of
replications.

The chance-floor case also shows that better fit need not imply a large
separation between models. AIC favored the chance-corrected model in
81\% of replications, with a median difference of 2.4 AIC units. The two
models were within two units in 39.0\% (117/300) of finite comparisons.
Among replications with a significant pseudo-interaction, all applicable
DHARMa checks were non-significant in 93.8\% (165/176) of replications.
...
GLMM. Its low sensitivity in the latter case therefore reflects limited
detection of that form of misspecification, not poor calibration. Full
calibration results are reported in Supplement A, and Supplement B shows
how pseudo-interaction rates, Pregibon detection, and AIC preference
change across sample size and scale location.
...
plausible links. Theory, task structure, and measurement should still
guide the preferred specification, and diagnostics can provide evidence
about model fit. The key robustness question, however, remains whether
the interaction conclusion survives across alternative plausible links.
Researchers should therefore compare the same target response-scale
contrast across alternatives and report whether the result is stable or
link-conditional. If it changes, that dependence is itself part of the
result (\citeproc{ref-mccabe2022}{McCabe et al., 2022};
...
Three limitations bound the scope of the conclusions. First, the review
...
interaction was scale-dependent enough to be removable. Finally, the
...
link-function curvature. Supplement B broadens these scenarios varying
the data-generating parameters, but it still does not estimate
prevalence or cover all real-life complications.

In conclusion, interaction effects are often tested while the scale of
additivity remains implicit. Link-aware testing separates three
questions that are often run together: which outcome family is
plausible, which link defines the scale of additivity, and whether the
interaction conclusion is stable across plausible alternatives. This
makes the scope of the claim explicit. A result that is stable across
defensible links is more robust. If it changes, the interaction should
be reported as conditional on the chosen scale.
