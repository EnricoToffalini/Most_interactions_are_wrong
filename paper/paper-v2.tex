...
a task, and errors decrease with age in both. Imagine a researcher who
collects error counts from children aged 6 to 10 and analyzes them, as
is common, with a Gaussian linear model that includes an age-by-group
interaction. The fitted lines in Figure~\ref{fig-simple-count-example}
(Panel A) decline at different rates, and the gap between the groups is
much wider at age six than at age ten. The natural reading is that the
group difference shrinks with age, that is, that age and group interact.
...
is whether the group difference itself changes with age. The data in
Figure~\ref{fig-simple-count-example}, however, were generated without
any interaction. Errors follow a Poisson process in which age and group
add on the log scale, so the two groups share the same age slope on that
scale. A Poisson model with a log link recovers this structure, and on
the log scale its fitted lines are parallel
(Figure~\ref{fig-simple-count-example}, Panel B). Mapped back onto the
count scale, the same model produces two curves that converge with age,
because a constant difference on the log scale is a constant ratio on
the count scale, and the absolute gap between the groups therefore
narrows as errors decline (Figure~\ref{fig-simple-count-example}, Panel
C). The linear model registers this narrowing and reads it as an
interaction. Whether an interaction is present thus depends on the scale
on which effects are assumed to add. Additivity on the log scale implies
non-parallel curves on the count scale, and additivity on the count
scale would imply non-parallel lines on the log scale.
...
\begin{tcolorbox}[enhanced jigsaw, colback=white, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, arc=.35mm, titlerule=0mm, bottomtitle=1mm, coltitle=black, breakable, opacitybacktitle=0.6, leftrule=.75mm, opacityback=0, title={Box 1: Generalized linear models: outcome family and link function}, colframe=quarto-callout-note-color-frame, rightrule=.15mm, bottomrule=.15mm, toprule=.15mm, left=2mm]
...
and colleagues separated removable from nonremovable interactions. A
removable interaction can be undone by a monotonic transformation of the
dependent variable, whereas a nonremovable one cannot. Recently, J. C.
Dunn and Anderson (\citeproc{ref-dunnanderson2026}{2026}) developed a
removability test for linear models, and Rohrer and Arslan
...
such as a threshold effect. In GLMs specifically, the product-term
coefficient refers to the link scale and does not equal the interaction
on the response scale, so a question about the response scale should be
answered with marginal effects (\citeproc{ref-ainorton2003}{Ai \&
...
question, interaction estimates can be unstable for other reasons such
as small effects, low reliability, and limited power
...
common response formats, namely forced-choice accuracy with a chance
floor, sum scores, and within-family link choices such as logit versus
probit. Simulations quantify how often an inadequate link turns a purely
...
call such a result a pseudo-interaction.
...
function, 29\%, 95\% CI {[}23.2, 35.6{]}, and only 46 made the link
function clearly identifiable, 23\%, 95\% CI {[}17.7, 29.3{]}. Thus, in
...
Gaussian-identity analysis on a constrained observed outcome, 72\%, 95\%
CI {[}65.4, 77.8{]}. As noted, this is a conservative screening count,
flagged only when the constraint was clear from the report. Of these 144
cases, 124 reported at least one statistically significant interaction,
86.1\%, 95\% CI {[}79.5, 90.8{]}. This is not a comparison, because
significance was coded only within the flagged analyses; that is, 62.0\%
of all interaction-testing articles combined a constrained outcome, an
identity-link analysis, and a significant interaction claim. This is the
same concern raised by work on outcome-model mismatch in interaction
...
the proportion of Gaussian-identity analyses on constrained observed
outcomes among interaction-testing articles ranged from 40.0\% in
Psychological Medicine to 95.0\% in Psychological Science. In this
sample, Psychological Medicine had the lowest screening proportion and
the highest non-identity-link proportion (40.0\%). This journal, which
uses many outcomes for which non-identity links are standard,
contributed the fewest such cases.
...
the scale on which the data-generating process is additive. Ideally, J.
C. Dunn and Anderson (\citeproc{ref-dunnanderson2026}{2026}) provide a
direct test of removability; however, that requires suitable cell-level
summaries and error estimates, which were often not reported in the
articles we reviewed; in addition, many designs use mixed-effects
models, whose output do not simply reduce to cell-level means. The
...
the standard logit rejects 60\% of the time, the standard probit 57\%,
and the Gaussian identity 44\%. Detection falls as performance moves
away from the floor, reaching 32\% for the logit, 19\% for the probit,
and 14\% for the Gaussian identity in the higher-performance scenario.
...
replication, including the lower-performance scenario. However under the
matched chance-corrected model, the Monte Carlo standard deviation of
the estimated age-by-group coefficient was 0.180 in the
...
every scenario, from 4\% to 5\%. The identity model behaved similarly
...
range and 73\% in the lower range. Compression of the latent scale makes
...
alpha across scenarios, from 4\% to 9\%. It respects the score bounds,
...
normal under an adequate model (\citeproc{ref-dunnsmyth1996}{P. K. Dunn
...
every one of the 3000 replications except in the count scenario, where
the Poisson-identity model returned a usable fit in 2798 of them. The
...
replications, but reached only 33\% in the Gamma case and 29\% at the
chance floor, fell to 5\% for the binary case with a continuous
predictor, and could not be computed at all in the saturated
\(2 \times 2\) design. The DHARMa checks were weaker, reaching 91\% in
...
AIC favored the target link in every scenario, from 78\% in the Gamma
...
the fewest pseudo-interactions, 10\% in the saturated design and 7\%
with a continuous predictor, and almost nothing detected them, leaving
AIC at 95\% and 95\% as the only informative signal.
...
misspecification. There AIC favored the chance-corrected model in 78\%
...
high. Under the correct link, DHARMa flagged a problem in at most 5.2\%
of replications. Pregibon flagged at most 5.4\% in the count,
chance-floor, and Gamma scenarios, and 4.4\% in the continuous-predictor
GLMM.
...
For factorial designs with cell-level data and an explicit
construct-level claim, the monotonic linear model (mLR) of J. C. Dunn
and Anderson (\citeproc{ref-dunnanderson2026}{2026}) provides a more
general test of scale dependence, asking whether additivity can be
recovered under any monotonic transformation. All our above simulated
pseudo-interactions are removable in this broader sense. However,
removability alone does not determine which transformations are
scientifically admissible: for many outcomes, the family, bounds, chance
levels, or substantive interpretation constrain the set of plausible
mappings; furthermore, the method is not currently implemented for mixed
models. Here, we only ask whether interaction conclusions are affected
by a choice among standard, plausible link functions.
