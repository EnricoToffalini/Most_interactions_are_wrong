...
All four models returned usable Wald tests in all 3,000 replications of every scenario, so a separate common-convergence table would duplicate @tbl-rejection-forced-choice. Complete fit success does not imply equal precision, however. For the matched chance-corrected model, the Monte Carlo standard deviation of the estimated age-by-group coefficient was 0.180 in the lower-performance scenario, 0.115 in the middle-performance scenario, and 0.099 in the higher-performance scenario. Thus, performance closer to the chance floor provides less precise information about changes on the above-chance link scale, even though the robust optimization procedure yields a usable test throughout. The lower-performance result should therefore be interpreted as reduced precision rather than selective convergence or widespread non-identification.
