...
\abstract{Interaction tests are a routine part of psychological
analysis, but an interaction claim is not defined by a product term
alone. It also depends on the scale on which additivity is assumed. This
scale problem is not new, but we reframe it in the modern GLM language
of outcome family and link function. We first report a preregistered
descriptive review of recent psychological articles, showing that
interaction tests are common and that family, link, and
scale-of-additivity choices are often implicit, especially for bounded,
discrete, ordinal, and sum-score outcomes. We then develop a
link-centered framework for distinguishing wrong-family, wrong-link, and
wrong-metric problems, and for separating interactions on the linear
predictor scale from interactions on the observed response scale. Worked
examples focus on forced-choice accuracy with a non-zero chance floor,
sum scores as bounded and discrete outcomes, and within-family link
choices such as logit versus probit. Simulations show how inadequate
links can generate pseudo-interactions, while diagnostic checks may have
limited power to identify the wrong link before the interaction becomes
statistically detectable. We do not argue that all interactions are
artifacts. We argue that many interaction claims are underspecified
unless researchers state, justify, and probe the scale on which no
...
Psychological theories often ask whether the effect of one variable
...
such claims has become part of the standard analytic toolkit in
psychology, sometimes almost a routine step after main effects have been
examined. This routine use is understandable, because interactions often
express important theoretical ideas. Yet an interaction claim should not
be interpreted automatically from the presence or absence of a product
term (\citeproc{ref-rimpler2026}{Rimpler et al., 2026};
...
increases? This formulation is intuitive, but it hides a crucial
assumption. A difference is always defined on some scale. An interaction
is a difference between differences, but differences are not scale-free.
...
rather than genuine psychological synergy
...
especially when outcomes are binary, count, censored, or noninterval
...
This paper focuses on one specific part of this broader problem: the
role of the link function in GLM interaction testing. In the standard
GLM architecture, the family and the link answer two different modeling
questions (\citeproc{ref-mccullaghnelder1989}{McCullagh \& Nelder,
1989}; \citeproc{ref-nelderwedderburn1972}{Nelder \& Wedderburn, 1972}).
The family answers: what kind of outcome am I modeling? It specifies the
support and sampling process of the response, for example binary
choices, counts, positive continuous times, or approximately Gaussian
scores. The link answers: on which scale do I assume that effects add?
It maps the conditional mean of the outcome to the linear predictor and
therefore defines the scale on which a model without a product term is
additive.

Choosing an appropriate family is therefore necessary, but not
sufficient, for interaction testing. Two models can respect the same
outcome family while defining different no-interaction baselines. For
response times, for example, a Gamma family may be plausible because the
outcome is positive, continuous, and right-skewed. Yet
\texttt{Gamma(link\ =\ "log")} assumes additivity in log mean response
times, which corresponds to proportional change on the millisecond
scale, whereas \texttt{Gamma(link\ =\ "identity")} assumes additivity
directly in milliseconds. Neither choice is automatically correct. The
...
The same logic applies to accuracy data, where the issue can be even
more visible. A binomial family may correctly represent the sampling
process of correct and incorrect responses, but the link still defines
the scale of additivity. A standard logit link maps the linear predictor
to the full 0-to-1 probability range. In a two-alternative forced-choice
task, however, the theoretically meaningful lower bound may be chance
performance, that is, .50. A chance-corrected link can encode this lower
asymptote directly. Thus, the distinction is not only between linear
models and GLMs. It is also between plausible links within a broadly
appropriate family.
...
response times. Equal steps on that target-link scale map to unequal
steps on the observed response scale. Conversely, the same observed
response values would not be equally spaced on an alternative link
scale. This is the central reason why link choice matters for
interactions: a model without a product term is additive only on the
scale defined by its link. When another plausible link is used, the same
response-scale pattern can imply a different no-interaction baseline and
therefore a different moderation claim.

Figure~\ref{fig-motivating-example} uses one simulated two-alternative
forced-choice accuracy dataset as a motivating example. Each participant
completes 50 trials, and the outcome is the proportion correct. The
predictors are age and group. The data-generating model includes an age
effect and a group effect, but no age-by-group term on the
chance-corrected logit scale, the scale on which the task has a .50
chance floor. The point of the figure is not that one can obtain any
conclusion by changing links. The point is narrower: the same data can
support different apparent moderation claims when models define
...
\caption{\label{fig-motivating-example}Same data, different
no-interaction baselines. Data were simulated from a 2-alternative
forced-choice accuracy task with 50 trials per participant and a .50
chance floor. The data-generating model included age and group effects,
but no age-by-group interaction on the chance-corrected 2-AFC logit
scale. Panel A shows the true response-scale pattern implied by that
model. Panel B shows one simulated dataset. Panel C fits the same data
with three models. The Gaussian identity model is simple and
interpretable on the observed accuracy scale, but it can produce
predictions outside the 0-to-1 range and ignores the binomial sampling
process. The standard binomial logit/probit model, shown here with a
logit link, respects the binomial process and the 0-to-1 range, but
assumes a lower asymptote at 0. The chance-corrected binomial link
incorporates the task's chance floor. The question is not only which
curve fits, but which scale defines the scientific null hypothesis of no
moderation.}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{figs/fig1-motivating-example.pdf}}
...
choices make interaction claims arbitrary. Some interactions will be
robust across plausible links and response-scale summaries. Others will
not. The problem is that many interaction claims are underspecified
unless researchers state which scale defines additivity, whether the
reported interaction is on the linear predictor scale or the observed
response scale, and whether alternative plausible links lead to the same
substantive conclusion. In this sense, link choice is not a technical
afterthought. It is part of the scientific claim.

The link-function problem should also be separated from two related
problems. A wrong-family problem occurs when the model does not respect
the outcome type, for example when counts or proportions are treated as
Gaussian identity outcomes without justification. A wrong-link problem
occurs when the family is broadly plausible, but the link defines an
inadequate scale of additivity, for example when a standard binomial
logit link is used for a forced-choice task with a non-zero chance
floor. A wrong-metric problem occurs when the observed score is treated
as if it were already an interval-scale measure of the target construct,
for example when a Likert item or sum score is interpreted without
considering its ordinal, bounded, or nonlinear relation to the latent
quantity. These problems can overlap, but they are not the same.

Our contribution is therefore narrow. We do not claim that link
functions have been ignored, nor that scale dependence is a new idea.
Instead, we argue that psychology lacks an explicit, interaction-focused
framework for choosing among plausible link functions and for
quantifying how wrong or inadequate link choices can generate
pseudo-interactions in realistic psychological settings. This framework
is especially important when researchers have already moved beyond a
simple linear model and chosen a broadly plausible GLM family, but have
not examined whether the link function encodes the scale of additivity
implied by their scientific question.

We develop this argument in three applied cases. The central case is
forced-choice accuracy with a non-zero chance floor. Here, a standard
binomial model may be right about the sampling process but wrong about
the lower bound. The second case is sum scores, which are common in
psychology but often discrete, bounded, skewed, and only indirectly
related to latent constructs. We do not argue that sum scores are
invalid. We argue that interaction claims based on sum scores require
scale-aware sensitivity analyses when additivity on the manifest
sum-score scale is not clearly justified, especially because ordinal and
metric analyses can diverge in ways that affect interactions
(\citeproc{ref-liddellkruschke2018}{Liddell \& Kruschke, 2018}). The
third case is within-family link choice, such as logit versus probit.
These links are often practically similar in the middle of the
probability range, but their differences can matter for interaction
claims near floors, ceilings, chance levels, or distributional tails.

We also discuss diagnostics, but with a limited claim. Link tests,
residual diagnostics, calibration checks, and predictive checks can
reveal important forms of misfit (\citeproc{ref-dunnsmyth1996}{Dunn \&
Smyth, 1996}; \citeproc{ref-hartig2024}{Hartig, 2024};
\citeproc{ref-pregibon1980}{Pregibon, 1980}). They should be part of
responsible modeling practice. However, diagnostics do not by themselves
certify the scientific validity of an interaction claim, and they may
not distinguish a wrong link from a missing nonlinearity, a missing
interaction, a wrong family, or a problematic measurement metric. For
this reason, we treat diagnostics empirically in one focused worked
example, asking whether the wrong link is detected in the same
conditions in which it can generate a false-positive interaction.

The rest of the paper proceeds as follows. First, we report a
preregistered descriptive review of current practice in psychological
interaction testing. This review asks whether the issue is only a
technical corner case or whether it matters for current practice.
Second, we define the link-function problem and distinguish interactions
on the linear predictor scale from interactions on the observed response
scale. Third, we examine forced-choice accuracy, sum scores, and
within-family link choice as applied cases where link assumptions can
matter for moderation claims. Fourth, we use simulations to quantify
when wrong or inadequate link choices generate pseudo-interactions.
Fifth, we evaluate diagnostics in one selected scenario to show what
they can and cannot detect. We close with a minimal workflow for
...
Is this a technical corner case, or does it matter for current
psychological practice? This question motivated our preregistered
descriptive review. The review was designed to establish whether the
link-function problem is relevant for the kinds of interaction analyses
that psychologists actually report. If interaction tests were rare, or
if researchers routinely stated the outcome family, link function, and
scale of additivity, the problem would be less urgent. If, instead,
interaction tests are common and these modeling choices are often
implicit, then many moderation claims are difficult to evaluate because
the no-interaction baseline is not fully specified.

The review should not be read as an audit of whether individual articles
were wrong. In many cases, published reports do not provide enough
information to reconstruct the authors' full modeling rationale. A paper
may report a model family but not the link, analyze a transformed or
aggregated outcome without stating the implied measurement scale, or
target an observed-scale estimand that could be defensible but is not
described in those terms. For the same reason, we did not systematically
code whether reported interactions were removable or nonremovable. That
distinction is central to the classical scale-dependence literature
(\citeproc{ref-cox1984}{Cox, 1984}; \citeproc{ref-loftus1978}{Loftus,
1978}; \citeproc{ref-wagenmakers2012}{Wagenmakers et al., 2012}), but
classifying it reliably from published reports alone would often require
information about the raw outcome, the assumed measurement scale, the
link or transformation, and the substantive scale of interest. Our
coding choice was therefore conservative. We asked what could be learned
from reported practice: whether interaction tests are frequent, whether
link functions are explicit, and whether many analyses involve outcomes
for which identity-link additivity is not self-evident.

This makes the review a substantive entry point for the rest of the
paper. The main issue is not that all reported interactions are
artifacts. The issue is that many interaction claims are hard to
interpret when the family, link, and scale of additivity remain
implicit. This focus on transparent analytic choices is consistent with
broader recommendations that quantitative manuscripts should report
modeling decisions clearly enough for readers to evaluate them
(\citeproc{ref-mustillo2018}{Mustillo et al., 2018}).

\subsection{Review Method}\label{review-method}

The review protocol was preregistered before the current manuscript was
written. It specified a descriptive review of recently published
psychological research articles, with four primary goals: estimating how
often empirical articles test interactions, how often
...
Articles were sampled from five journals selected to cover different
areas of psychology: \emph{Psychological Science}, \emph{Journal of
Experimental Psychology: General}, \emph{Journal of Personality and
Social Psychology}, \emph{Developmental Science}, and
\emph{Psychological Medicine}. Eligible articles were empirical articles
presenting original psychological research. Reviews, meta-analyses,
editorials, theoretical articles without empirical data,
qualitative-only articles, replication studies, and psychometric
validation studies were excluded. Articles were then coded in detail if
they tested at least one interaction effect, either through statistical
...
which identity-link additivity was not obviously innocuous. Among the
200 articles testing interactions, 144 were coded as including at least
one potentially problematic identity-link analysis, 72\%, 95\% CI
{[}65.4, 77.8{]}. This code should be interpreted cautiously. It means
that, given the reported outcome and model, another link appeared
plausibly more appropriate. It does not prove that the specific
interaction claim was false. Among these 144 cases, 124 reported at
least one statistically significant interaction, 86.1\%, 95\% CI
{[}79.5, 90.8{]}. Table~\ref{tbl-current-practice} summarizes these
descriptive results.
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.6947}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0305}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0916}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0611}}
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.1221}}@{}}
\toprule\noalign{}
...
& 200 & 23.0\% & {[}17.7\%, 29.3\%{]} \\
Interaction-testing articles with a potentially problematic
identity-link analysis & 144 & 200 & 72.0\% & {[}65.4\%, 77.8\%{]} \\
Potentially problematic identity-link cases reporting at least one
...
The next section develops the link-function problem directly. We first
distinguish the outcome family from the link function, then show why
choosing the family is not enough for interaction testing, and finally
separate interactions on the linear-predictor scale from interactions on
...
link, and scale of additivity are often left implicit. This section
explains why that matters. In a generalized linear model (GLM), the
expected outcome for observation \(i\), denoted \(\mu_i = E(Y_i)\), is
related to a linear predictor through a link function
...
linear predictor is usually written as

\[
\eta_i = \beta_0 + \beta_1 X_i + \beta_2 Z_i + \beta_3 X_i Z_i.
\]

This notation separates two decisions that are often blurred. The family
answers the question: what kind of outcome am I modeling? It specifies
the support and sampling process of the response, such as Gaussian
scores, binomial successes, Poisson counts, or positive continuous
outcomes. The link answers a different question: on which scale do I
assume effects add? It maps the conditional mean of the outcome to the
linear predictor. Thus, choosing the family is necessary but not
sufficient. A plausible family can keep predictions in the right range,
but the link still defines the scale on which a model without a product
term is additive.

\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, arc=.35mm, opacitybacktitle=0.6, breakable, leftrule=.75mm, toprule=.15mm, coltitle=black, titlerule=0mm, rightrule=.15mm, left=2mm, bottomtitle=1mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{What the link function does}, bottomrule=.15mm, opacityback=0, colback=white]
...
are combined on this scale. Many psychological outcomes live on a
different scale. Counts cannot be negative. Probabilities must lie
between 0 and 1. Forced-choice accuracy may have a theoretical floor
above 0, for example .50 in a true/false task. Response times are
positive and often right-skewed. Sum scores have lower and upper bounds,
and are often discrete.
...
changes in \(\eta\) can become unequal changes in probabilities, counts,
milliseconds, or score units. Conversely, equal observed-scale
...
on the linear predictor scale is also additivity on the expected
response scale. For nonlinear links, this equivalence disappears. A
logit link defines additivity in log odds. A probit link defines
additivity on a latent-normal scale. A log link defines additivity in
log means, which corresponds to multiplicative change on the response
scale. The same general point applies within a family. As noted in the
introduction, a Gamma model for response times may use an identity, log,
or inverse link. The family may be plausible in all cases, but the
implied no-interaction baseline is different.

\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, arc=.35mm, opacitybacktitle=0.6, breakable, leftrule=.75mm, toprule=.15mm, coltitle=black, titlerule=0mm, rightrule=.15mm, left=2mm, bottomtitle=1mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, title={A simple count example}, bottomrule=.15mm, opacityback=0, colback=white]

Suppose children make fewer errors as they get older. A Poisson model
with a log link can represent this as a linear age effect on the log
mean,
...
mechanism. It is the expected consequence of mapping an additive model
on the log scale back to non-negative counts.

Now add a group difference in overall error rate, but no age-by-group
product term on the log scale. On the observed count scale, the absolute
age-related reduction will usually be larger where expected errors are
high and smaller where expected errors are low. A Gaussian identity
model fitted to raw counts may treat this changing absolute difference
as moderation. A Poisson-log model can describe the same pattern as
additive main effects on the log-count scale. The point is not that
Poisson-log is always correct. The point is that product-term
coefficients and response-scale contrasts answer different questions for
counts and other non-Gaussian outcomes
(\citeproc{ref-domingue2024}{Domingue et al., 2024};
\citeproc{ref-mccabe2022}{McCabe et al., 2022}).
...
The link-function problem is therefore a scale-of-additivity problem. To
ask whether \(X\) and \(Z\) interact is to ask whether their effects
depart from additivity on some scale. An identity-link model asks
whether they depart from additivity on the expected observed mean. A
binomial-logit model asks whether they depart from additivity in log
odds. A binomial-probit model asks whether they depart from additivity
on a probit scale. A chance-corrected binomial link asks whether they
depart from additivity on a scale that maps the linear predictor onto
the interval from chance performance to perfect performance. These are
related statistical models, but they do not express the same scientific
null.
...
{\caption{{Three distinct sources of scale-related problems for
interaction claims.}{\label{tbl-family-link-metric}}}
\vspace{-20pt}}
...
\bottomrule\noalign{}
\endlastfoot
Wrong family & Does the model represent the support or sampling process
of the outcome? & Counts, proportions, or trial accuracies modeled as
unbounded Gaussian observations. & The fitted model may impose
impossible predictions or miss the mean-variance relation, so curvature
or constraints can appear as moderation. \\
Correct family, wrong link & Given the outcome family, which scale
defines additivity? & Forced-choice accuracy modeled with a standard
binomial logit, even though the task has a known chance floor such as
.50. & The product term tests additivity on the wrong scale, so a
pseudo-interaction can arise even when the correct link-scale model has
only main effects. \\
Wrong measurement metric & Does the recorded score have an interpretable
interval scale for the claim being made? & Likert items or sum scores
treated as raw interval outcomes without justification. & A product term
may reflect thresholds, discreteness, bounds, or scale compression
rather than moderation of the underlying construct. \\
\end{longtable}

\end{table}

The present paper focuses mainly on the second problem, while keeping
the other two visible. This focus is narrower than the general warning
that linear models can be inappropriate for non-Gaussian outcomes.
Researchers may choose a broadly appropriate family and still leave the
scientific scale of additivity unspecified. For instance, a binomial
family may correctly represent successes out of trials, but it does not
choose among logit, probit, complementary log-log, flexible generalized
links, or a chance-corrected link with a lower asymptote
...
For these tasks, the theoretically relevant response range is often not
0 to 1, but chance to 1.
...
still defines the lower asymptote as 0. A standard binomial model is
right about the sampling process, but wrong about the lower bound.
...
The difference matters most when one group, condition, or region of the
predictor space lies close to chance. Suppose that age has the same
...
accuracy scale, the lower-performing group can still show compression
near .50, simply because there is little meaningful room below chance. A
standard logit or probit link places this lower compression near 0
rather than near .50. The fitted model can then use an age-by-group
product term to absorb systematic curvature created by the misplaced
lower bound. The resulting interaction is not merely random Type I
error. It is a pseudo-interaction produced by a mismatch between the
scale used by the model and the scale implied by the task. This is
consistent with broader work showing that outcome-model misspecification
can create false discoveries in interaction analyses
...
approximately continuous psychological dimension. But the latent trait
and the observed score are different objects. The latent dimension may
be approximately continuous and roughly normal, whereas the observed
score is produced by a finite set of item responses through thresholds,
endorsement probabilities, success probabilities, or category labels.
The mapping from latent trait to observed total is therefore discrete,
bounded, and often nonlinear.

This point matters even when the construct itself is well behaved. Even
if the underlying psychological dimension were approximately normal, the
sum score need not be. A total based on a small number of binary or
ordinal items has only a limited number of possible values. It has a
lower and upper bound. It can be skewed when items are too easy, too
hard, rarely endorsed, or almost always endorsed. It can be compressed
near floor or ceiling because there is little room left for the score to
move. It can also be non-interval when adjacent raw-score values do not
correspond to equal changes in the latent construct. These features are
common in psychological variables (\citeproc{ref-micceri1989}{Micceri,
1989}), and they are central to why treating ordinal responses as metric
can produce false alarms, missed effects, reversals, and misleading
interactions (\citeproc{ref-liddellkruschke2018}{Liddell \& Kruschke,
2018}).
...
explicit. None is automatically correct, and each should be tied to the
target claim.
...
\bottomrule\noalign{}
\endlastfoot
Cumulative-link or ordered-probit/logit model & It treats the outcome as
ordered categories and models threshold crossing rather than equal
raw-score distances (\citeproc{ref-agresti2010}{Agresti, 2010};
\citeproc{ref-burknervuorre2019}{Bürkner \& Vuorre, 2019};
\citeproc{ref-smith2020}{Smith et al., 2020}). & It can be awkward for
long totals, may rely on proportional-odds or related assumptions, and
may still be too coarse if the claim concerns item-level functioning. \\
Item-level ordinal or IRT-inspired model & It separates the latent
construct from item thresholds, item difficulties, and item
discrimination, so floor, ceiling, and uneven item sensitivity can be
represented directly. & It needs item-level data, stronger measurement
assumptions, larger samples, and attention to local dependence,
differential item functioning, and model fit. \\
Binomial, beta-binomial, or Poisson-binomial approximation & It can be
useful when the score is a count of successes, symptoms, or
endorsements, and it respects discreteness and bounds. Beta-binomial
versions can absorb overdispersion, and Poisson-binomial versions can
allow heterogeneous item probabilities. & A simple binomial model may
wrongly treat items as exchangeable trials. These models may also be
poor for ordinal ratings or for items that do not define comparable
successes. \\
Robust and sensitivity analyses & They show whether the interaction
conclusion depends on the manifest sum-score metric, for example by
comparing raw-score, ordinal, bounded-count, and item-level analyses. &
They do not identify the true scale by themselves. If plausible analyses
disagree, the substantive conclusion must be stated as
scale-dependent. \\
\end{longtable}
...
\pandocbounded{\includegraphics[keepaspectratio]{figs/fig4-logit-probit-fitted-example.pdf}}
...
Diagnostics should be part of link-aware modeling, but they are not a
complete solution to the link-function problem. The reason is a
diagnostic paradox. A wrong link can induce a systematic
pseudo-interaction that becomes statistically detectable as sample size
increases, while the diagnostics used to identify the wrong link may
still have limited power, or may flag only a broad form of misfit. In
that situation, the interaction test can become sensitive to the
artifact before the diagnostic evidence becomes specific enough to
identify the source of the artifact.

Pregibon-style link tests are one direct response to this problem. They
expand the fitted GLM by adding terms derived from the fitted link, then
ask whether the expanded model improves the fit
(\citeproc{ref-pregibon1980}{Pregibon, 1980}). This is useful because
the test targets link adequacy more directly than a generic residual
plot. Its limitation is also central. A significant added-term test is a
warning that the assumed link may be inadequate, but it does not
automatically identify the correct link. The signal may also reflect a
missing nonlinear predictor, an omitted covariate, an omitted
interaction, or another misspecification of the systematic component.
Conversely, a non-significant result does not certify that the link
defines the scientifically correct scale of additivity. It only means
...
Smyth, 1996}; \citeproc{ref-hartig2024}{Hartig, 2024}). The resulting
residuals are designed to be interpretable across GLMs and GLMMs, where
raw, Pearson, and deviance residuals can be difficult to read. In the
worked example below, we use four DHARMa checks: residual uniformity,
dispersion, residual quantile patterns over fitted values, and residual
quantile patterns over age. These checks are useful because they can
detect broad misfit, overdispersion or underdispersion, zero-inflation
or zero-deflation, and structured residual patterns. Some diagnostics
may detect some forms of link-induced misfit, as suggested by dispersion
checks in related logit-probit examples. But this depends on the
scenario, sample size, outcome distribution, and type of departure. A
residual diagnostic that works in one case should not be treated as a
general certificate of link adequacy.

Model comparison tools, such as AIC, BIC, WAIC, or LOO, add another
source of evidence. They can compare candidate models when the candidate
set is meaningful and specified before interpreting the interaction.
However, relative fit is not the same as scientific scale validity. Two
links can have similar predictive performance while implying different
no-interaction baselines, especially near floors, ceilings, chance
levels, or distributional tails. Conversely, a model with slightly
better predictive fit may not correspond to the estimand that the theory
requires. For interaction claims, predictive adequacy and substantive
adequacy are related, but they are not identical.

For this reason, diagnostics should be treated as warnings, not
certification. They can show that a model deserves scrutiny, and
sometimes they can show that a fitted model is plainly inadequate. They
usually cannot distinguish, by themselves, between a wrong link, a
missing nonlinearity, a missing interaction, a wrong family, and a wrong
measurement metric. The practical implication is that diagnostics should
be paired with link sensitivity analyses and an explicit definition of
the estimand. If the scientific claim concerns response-scale
moderation, researchers should report response-scale contrasts or
marginal effects and examine whether those quantities are stable across
plausible links (\citeproc{ref-mccabe2022}{McCabe et al., 2022};
...
linear-predictor scale, the chosen link needs a theoretical
justification. Diagnostics can support that argument, but they cannot
replace it.
...
\begin{tcolorbox}[enhanced jigsaw, colframe=quarto-callout-note-color-frame, arc=.35mm, opacitybacktitle=0.6, breakable, leftrule=.75mm, toprule=.15mm, coltitle=black, titlerule=0mm, rightrule=.15mm, left=2mm, bottomtitle=1mm, toptitle=1mm, colbacktitle=quarto-callout-note-color!10!white, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{Pseudo-interactions as power against the wrong null}, bottomrule=.15mm, opacityback=0, colback=white]
...
Under a wrong link, the same data-generating process may imply a nonzero
interaction on the fitted link scale. In that case, a significant
product term is better understood as power to detect a
pseudo-interaction created by the mismatch between the generating link
and the fitted link.
...
connects it to GLM misspecification. Prior work has shown that
outcome-model misspecification can produce biased and false-positive
interaction findings (\citeproc{ref-domingue2024}{Domingue et al.,
2024}), and that product-term coefficients in nonlinear GLMs do not
automatically equal interaction effects on the response scale
(\citeproc{ref-ainorton2003}{Ai \& Norton, 2003};
\citeproc{ref-mccabe2022}{McCabe et al., 2022};
\citeproc{ref-mize2019}{Mize, 2019}). Here, the same logic is used
prospectively: if a wrong link induces a nonzero interaction on the
fitted scale, the expected false-interaction rate can be understood as
the probability of detecting that induced quantity.

The idea is easiest to see in a simple 2 × 2 design. Let
\(\mu_{00}, \mu_{10}, \mu_{01}, \mu_{11}\) be the four expected response
means generated from a GLM with no product-term interaction on the
target link scale. If the fitted model uses link \(g\), the induced
interaction on that fitted scale is
...
target link, \(\Delta_g\) can be different from zero even though the
generating model contains no product term. In this 2 × 2 case,
\(\Delta_g\) is the population link-scale difference-in-differences that
the fitted product term is trying to estimate. The probability of a
significant interaction is then governed by the size of this
pseudo-effect relative to its standard error, approximately
\(\lambda = \Delta_g / SE(\hat{\Delta}_g)\). This is why the risk
depends jointly on the two main effects, the intercept or baseline
location, the range of the predictors, the shape of the link, and the
precision of the design.

This framing also explains why link misspecification is not equally
harmful everywhere. If predicted values remain in a range where
plausible links produce nearly the same mean structure, \(\Delta_g\) may
be negligible. Near floors, ceilings, chance levels, or distributional
tails, the same main effects can generate a much larger
pseudo-interaction. The simulations below estimate this
misspecification-induced detection rate in realistic psychological
designs.
...
Second, in a diagnostic worked example, we estimate how often model
checks flag the misspecification that produced the pseudo-interaction.
This comparison matters because link-oriented tests and simulation-based
residual diagnostics can be useful, but they do not guarantee that the
link is adequate for the interaction claim being tested
...
\pandocbounded{\includegraphics[keepaspectratio]{figs/fig2-forced-choice-simulation.pdf}}
...
The bounded binomial approximation treats the score as a bounded count,
which respects the 0-to-maximum range but remains only an approximation
because item responses are ordinal and heterogeneous. The latent
generating scale is an idealized benchmark available only in the
...
\pandocbounded{\includegraphics[keepaspectratio]{figs/fig3-sum-score-simulation.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{figs/fig4-within-family-links.pdf}}
...
\subsection{Diagnostic Worked Example}\label{diagnostic-worked-example}

Finally, we examined diagnostics in one selected chance-floor scenario.
The data were generated from a chance-corrected binomial model with a
.50 lower asymptote and no age-by-group product term on the generating
link scale. We then fitted the misspecified standard binomial-logit
model, which respects the binomial sampling process but assumes a lower
asymptote of 0. The purpose was not to find the best possible diagnostic
battery. It was to compare two rates in the same Monte Carlo setting:
the false-positive rate for the age-by-group interaction under the wrong
link, and the rate at which diagnostic checks flag the same wrong-link
problem.

The primary diagnostic workflow used DHARMa residual checks for the
fitted standard binomial-logit model (\citeproc{ref-dunnsmyth1996}{Dunn
\& Smyth, 1996}; \citeproc{ref-hartig2024}{Hartig, 2024}). In each
replication, we simulated randomized quantile residuals and recorded
four p-values: residual uniformity, dispersion, residual quantile
patterns over fitted values, and residual quantile patterns over age. We
report these checks separately. We do not combine them into a single
Holm-corrected or uncorrected omnibus decision, because the diagnostics
are correlated, partly redundant summaries of the same simulated
residuals and have different meanings. Uniformity, dispersion, and
residual quantile patterns do not answer the same diagnostic question. A
single corrected omnibus decision would therefore be too easy to misread
as a general pass-fail test of link adequacy.

We also report a Pregibon-style added-term link check as a secondary
comparator (\citeproc{ref-pregibon1980}{Pregibon, 1980}). This check is
closer to the link-function problem than a generic residual diagnostic,
but it still tests only a restricted alternative. Its role here is not
to certify the correct link. Its role is to show whether a link-oriented
diagnostic detects the misspecification in the same replications in
which the wrong link can produce a false-positive interaction.

\begin{table}

{\caption{{Diagnostic worked example: false-positive interaction rate
and separate diagnostic detection rates in one selected chance-floor
scenario. CI = Wilson 95\% confidence
interval.}{\label{tbl-diagnostic-worked-example}}}
\vspace{-20pt}}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedleft\arraybackslash}p{(\linewidth - 8\tabcolsep) * \real{0.0513}}
...
DHARMa uniformity & 300 & 1 & 0.003 & {[}0.001, 0.019{]} \\
DHARMa dispersion & 300 & 1 & 0.003 & {[}0.001, 0.019{]} \\
DHARMa residual quantiles over fitted values & 300 & 52 & 0.173 &
{[}0.135, 0.220{]} \\
DHARMa residual quantiles over age & 300 & 89 & 0.297 & {[}0.248,
0.351{]} \\
Pregibon-style added-term link check, secondary & 300 & 179 & 0.597 &
{[}0.540, 0.651{]} \\
\end{longtable}
...
\caption{\label{fig-diagnostic-worked-example}Diagnostic worked example.
Panel A shows the deterministic generating pattern: a chance-corrected
binomial model with a .50 floor and no age-by-group product term on the
generating link scale. Panel B shows the false-positive rate for the
age-by-group interaction after fitting the wrong standard binomial-logit
link. Panel C shows diagnostic detection rates reported separately:
DHARMa uniformity, DHARMa dispersion, DHARMa residual quantiles over
fitted values, DHARMa residual quantiles over age, and a Pregibon-style
...
\pandocbounded{\includegraphics[keepaspectratio]{figs/fig5-diagnostic-worked-example.pdf}}

}

\end{figure}%

The example illustrates the diagnostic paradox. The misspecified
standard binomial link can produce a detectable interaction even though
the generating model contains no age-by-group product term on the
appropriate link scale. Some diagnostics may flag the problem, and this
should be valued when it happens. In this scenario, however, the
diagnostic signals are not equivalent, and none of them should be
interpreted as a definitive test of link correctness. The Pregibon-style
check is informative because it is link-oriented, but it still tests a
narrow added-term alternative. DHARMa checks are useful because they
describe different residual departures, but they do not certify the
substantive scale of the interaction. The main lesson is therefore
pragmatic: diagnostics are warnings, not certification. For interaction
claims, they should be accompanied by a clear estimand, a justified
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\linewidth - 6\tabcolsep) * \real{0.2827}}
...
Question
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Why it matters for interactions
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Minimal reporting
\end{minipage} \\
\midrule\noalign{}
...
\bottomrule\noalign{}
\endlastfoot
Define the interaction estimand & What quantity would answer the
moderation question? & Product terms, marginal effects, and
difference-in-differences can answer different questions. & State the
target contrast, scale, moderator values, and whether the estimand is
descriptive, associational, or causal. \\
Inspect outcome constraints & What restrictions does the outcome impose?
& Bounds, discreteness, floors, ceilings, and chance levels can make
equal observed differences misleading. & Report the response range,
discreteness, trial or item count, chance level, and visible floor or
ceiling effects. \\
Justify the outcome family & What sampling model and support are
plausible? & A wrong family can create residual structure and
pseudo-interactions before link choice is considered. & Name the family
and justify it from the outcome type and design, such as trials, counts,
positive times, or ordered categories. \\
Justify the link & On which scale should effects add if there is no
interaction? & The link defines the no-interaction baseline. A wrong
link can turn curvature into a product term. & Name the link and give
the substantive or design reason, such as odds, latent threshold, log
mean, or chance floor. \\
Distinguish linear-predictor interaction from response-scale interaction
& Is the claim about the model scale, the observed scale, or both? & A
significant product coefficient may not correspond to meaningful
response-scale moderation. & Report the product coefficient when
relevant, and also predicted means, contrasts, marginal effects, or
difference-in-differences. \\
Run diagnostics & Do model checks suggest family, link, or
functional-form misfit? & Misfit can mimic moderation, but diagnostics
rarely identify one unique cause. & Report the checks used, what they
were intended to detect, and the result. Avoid saying only that the
model passed. \\
Run link sensitivity analyses & Does the interaction conclusion change
under plausible links? & Robust conclusions are easier to defend.
Unstable conclusions should be framed as link-dependent. & Report the
alternative links, why they were plausible, and whether the sign,
magnitude, or inference changed. \\
Report limitations & Which assumptions remain unresolved? & Some link
and metric choices cannot be settled by the data, especially in typical
sample sizes. & State the remaining assumptions and how they limit the
substantive interaction claim. \\
\end{longtable}

\end{table}

The workflow is not a rule that can make interaction testing automatic.
It is a way to put the main judgments in the open. Researchers still
need theory to decide which contrast matters, which scale is meaningful,
and which links are plausible. The table simply asks that these
decisions be visible before the product term is interpreted. This
matters because different quantities can be defensible for different
questions: a linear-predictor coefficient may be useful for a
model-based claim, whereas a response-scale contrast or marginal effect
may be closer to the substantive claim made in words
(\citeproc{ref-mccabe2022}{McCabe et al., 2022};
\citeproc{ref-mize2019}{Mize, 2019}; \citeproc{ref-rohrer2026}{Rohrer \&
Arel-Bundock, 2026}). Diagnostics and predictive checks can warn about
misspecification, but they do not prove that the selected link is the
true scientific scale (\citeproc{ref-dunnsmyth1996}{Dunn \& Smyth,
1996}; \citeproc{ref-hartig2024}{Hartig, 2024};
\citeproc{ref-pregibon1980}{Pregibon, 1980}). Sensitivity analyses
therefore play a pragmatic role. When the interaction conclusion is
stable across plausible links, the claim becomes easier to defend. When
it is not stable, the paper can still be informative, but the conclusion
should be stated as conditional on a modeling choice. In this sense,
link-aware reporting extends general quantitative-reporting principles:
choices that affect the claim should be visible, justified, and
interpretable (\citeproc{ref-mustillo2018}{Mustillo et al., 2018}).
...
will be robust across reasonable links, scales, and response-scale
summaries. Others will not. Our claim is narrower: many interaction
claims are underspecified because they do not state the scale on which
additivity, and therefore no interaction, is being assumed. In GLMs,
this scale is defined by the link function. This makes link choice a
substantive modeling decision, not only a technical detail. The same
issue may also extend beyond interactions to the broader study of
nonlinear effects, because many psychological analyses search for
curvature, thresholds, and changing slopes without first specifying the
scale on which these patterns are expected to be absent or present
...
The conceptual framework separated three problems that are often mixed
together: wrong family, wrong link, and wrong measurement metric.
Forced-choice accuracy illustrates the wrong-link problem clearly. A
standard binomial family respects trial counts, but a standard link may
fail to encode the non-zero chance floor of the task. Sum scores
illustrate the measurement-metric problem: an interaction on the
manifest score scale may not be an interaction on the latent construct
scale. Within-family links illustrate the more general point that
plausible links can define different baselines for no interaction.
...
diagnostic example shows a complementary point: diagnostics are useful,
but they do not by themselves certify that a scientific interaction
claim is made on the intended scale.

We deliberately reported the DHARMa checks separately rather than
collapsing them into a single omnibus decision. In applied work,
residual diagnostics are rarely equivalent to one clean hypothesis test.
They are better understood as partially redundant probes of different
possible symptoms of misspecification. This is precisely why passing one
diagnostic, or even several diagnostics, should not be taken as strong
evidence that an interaction claim is robust to link choice.
...
to very similar conclusions, especially away from boundaries; the
paper's claim is that link choice can matter for interaction claims, not
that it always does.
...
plausible alternatives. When the same conclusion holds across defensible
...
conditional on a modeling choice. This workflow complements existing
guidance on nonlinear interactions, marginal effects, and
prediction-based interpretation (\citeproc{ref-mccabe2022}{McCabe et
al., 2022}; \citeproc{ref-mize2019}{Mize, 2019};
\citeproc{ref-rohrer2026}{Rohrer \& Arel-Bundock, 2026}). In GLMs, the
link function is not an afterthought. It is the scale on which no
...
ed.). Wiley. \url{https://doi.org/10.1002/9780470594001}
