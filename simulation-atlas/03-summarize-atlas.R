...
diagnostic_rows <- lapply(seq_len(nrow(diagnostic_plan)), function(i) {
    ...
    applicability <- data.frame(diagnostic = diagnostic_names,
      applicable = if (scenario$family == "forced_choice") c(TRUE, TRUE, TRUE, TRUE, TRUE, FALSE, TRUE) else if (scenario$family == "within_family") c(TRUE, TRUE, TRUE, FALSE, FALSE, TRUE, TRUE) else rep(FALSE, 7))
