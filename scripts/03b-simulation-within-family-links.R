...
# Simulation 2: within-family link choice with repeated binary trials.
...
for (generating_link in settings$candidate_links) {
  ...
  g$generating_link_label <- paste0("Generated with ", ifelse(generating_link == "logit", "Logit", "Probit"), " link")
...
# ---------------------------------------------------------------------
# 3. Data generation, explicit model fits, and Monte Carlo simulation
# ---------------------------------------------------------------------
...
cell_plot_data$generating_link_label <- factor(cell_plot_data$generating_link_label, levels = paste0("Generated with ", ifelse(settings$candidate_links == "logit", "Logit", "Probit"), " link"))
  ...
  lapply(settings$candidate_links, function(link) {
      ...
      data.frame(
        generating_link_label = paste0(
          "Generated with ", ifelse(link == "logit", "Logit", "Probit"), " link"
        ),
...
link_grid$generating_link_label <- factor(
  ...
  levels = paste0(
    "Generated with ",
    ifelse(settings$candidate_links == "logit", "Logit", "Probit"),
    " link"
  )
)
...
p_scenario <- ggplot2::ggplot(
  ...
  ggplot2::scale_y_continuous(limits = c(0, 1), labels = function(x) paste0(round(100 * x / 1) * 1, "%")) +
...
plot_summary$generating_link_label <- factor(paste0("Generated with ", ifelse(as.character(plot_summary$generating_link) == "logit", "Logit", "Probit"), " link"), levels = paste0("Generated with ", ifelse(settings$candidate_links == "logit", "Logit", "Probit"), " link"))
plot_summary$fitted_link_label <- factor(ifelse(as.character(plot_summary$fitted_link) == "logit", "Logit", "Probit"), levels = ifelse(settings$candidate_links == "logit", "Logit", "Probit"))
...
p_rejection <- ggplot2::ggplot(plot_summary, ggplot2::aes(x = fitted_link_label, y = rejection_rate, color = link_match, shape = link_match)) +
  ...
  ggplot2::facet_wrap(~ generating_link_label, nrow = 1) +
  ggplot2::scale_y_continuous(labels = function(x) paste0(round(100 * x / 1) * 1, "%")) +
  match_scales +
  ggplot2::labs(
...
plots <- list(p_scenario, p_rejection)
plot_columns <- 1
...
for (plot_format in c("pdf", "png")) {
  if (plot_format == "pdf") {
    grDevices::pdf(paste0(settings$figure_base, ".pdf"), width = figure_width, height = 6.5)
  } else {
    grDevices::png(paste0(settings$figure_base, ".png"), width = figure_width, height = 6.5, units = "in", res = default_dpi)
  }
  grid::grid.newpage()
...
pseudo_plot_data$generating_link_label <- factor(paste0("Generated with ", ifelse(pseudo_plot_data$generating_link == "logit", "Logit", "Probit"), " link"), levels = paste0("Generated with ", ifelse(settings$candidate_links == "logit", "Logit", "Probit"), " link"))
pseudo_plot_data$fitted_link_label <- factor(ifelse(pseudo_plot_data$fitted_link == "logit", "Logit", "Probit"), levels = ifelse(settings$candidate_links == "logit", "Logit", "Probit"))
...
dir.create(dirname(settings$inspection_pseudo_base), recursive = TRUE, showWarnings = FALSE)
ggplot2::ggsave(paste0(settings$inspection_pseudo_base, ".pdf"), plot = p_pseudo, width = figure_width, height = 3.8)
ggplot2::ggsave(paste0(settings$inspection_pseudo_base, ".png"), plot = p_pseudo, width = figure_width, height = 3.8, dpi = default_dpi)
